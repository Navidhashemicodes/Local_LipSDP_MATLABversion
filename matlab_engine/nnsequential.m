...
%                 obj.weights{i} = U*S*V';
        ...
        function [alpha_param , beta_param] = slope_bounds(obj, center, epsilon)
            if(strcmp(obj.activation,'relu'))
                [l,u]                       = piecewise_linear_bounds_ReLU(obj.weights,obj.biases,center,epsilon);
                [alpha_param,beta_param]    = piecewise_linear_slopes_ReLU(l,u);
            elseif(strcmp(obj.activation,'tanh'))
                [l,u]                       = piecewise_linear_bounds_tanh(obj.weights,obj.biases,center,epsilon);
                [alpha_param,beta_param]    = piecewise_linear_slopes_tanh(l,u);
            else
                error('We have only included relu and tanh. In case, you need another activation, please easilly expand the toolbox by including that activation')
            end
        end
